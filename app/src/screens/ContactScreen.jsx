...
import { ArrowLeft, User, Building2, Hash, Shield } from 'lucide-react';
import Btn from '../components/Btn';
import Field from '../components/Field';
import { C, T } from '../tokens';

export default function ContactScreen({ onBack, onContinue }) {
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: C.white }}>
      ...
      <div style={{ flex: 1, padding: '0 24px', display: 'flex', flexDirection: 'column', gap: '18px', overflowY: 'auto' }}>
        <Field label="Nombre del destinatario" value={form.name}    onChange={set('name')}    placeholder="Ej: María García"        icon={User}      />
        <Field label="Banco"                   value={form.bank}    onChange={set('bank')}    placeholder="Ej: Bank of America"     icon={Building2} />
        <Field label="Número de cuenta / CBU"  value={form.account} onChange={set('account')} placeholder="Ej: 0000 1234 5678 9012" icon={Hash}      />
      ...
      <div style={{ padding: '24px', flexShrink: 0 }}>
        <Btn label="Continuar" onClick={() => onContinue(form)} variant="dark" disabled={!valid} />
      </div>
    </div>
  );
}
